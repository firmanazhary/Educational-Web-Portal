...
export default function AppLayout({ children, title }) {
    ...
    // Scroll listener navbar untuk efek glassmorphism
    useEffect(() => {
        const handleScroll = () => {
    ...
    const navLinks = [
        ...
        },
        {
            ...
            href: '/jenjang',
            children: [
                { name: 'PG (Playgroup)', href: '/pg', desc: 'Pendidikan usia dini 3 tahun' },
                { name: 'TK (Taman Kanak-Kanak)', href: '/tk', desc: 'Pendidikan usia 4-6 tahun' },
                { name: 'SD IT', href: '/sd', desc: 'Sekolah Dasar Islam Terpadu' },
                { name: 'SMP IT', href: '/smp', desc: 'Sekolah Menengah Pertama IT' },
                { name: 'SMA IT', href: '/sma', desc: 'Sekolah Menengah Atas IT' },
            ]
        },
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-1.5">
                                {navLinks.map((link) => {
                                    const isActive = url === link.href || (link.href !== '/' && url.startsWith(link.href));
                                    const hasChildren = link.children && link.children.length > 0;
                                    const isSubOpen = mobileDropdown === link.name;
                                    ...
                                    if (hasChildren) {
                                        return (
                                            <div key={link.name} className="py-1">
                                                ...
                                                >
                                                    <span>{link.name}</span>
                                                    <ChevronDown size={16} className={`transition-transform duration-200 ${isSubOpen ? 'rotate-180 text-[#FFC72C]' : 'text-slate-400'}`} />
                                                </button>
